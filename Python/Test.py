    ...
    row1.append(
        InlineKeyboardButton(
            ...
            callback_data="{} {} {}".format(
                "settings", update.message.from_user.id, "Name"
            ),
        )
    )
    row1.append(
        InlineKeyboardButton(
            "Stufe",
            callback_data="{} {} {}".format(
                "settings", update.message.from_user.id, "Stufe"
            ),
        )
    )
    row1.append(
        InlineKeyboardButton(
            "Erinnerung",
            callback_data="{} {} {}".format(
                "settings", update.message.from_user.id, "Erinnerung"
            ),
        )
    )
...
def start(bot, update):
    ...
    if get_person_value(id, "ID") is None:
        bot.send_message(
            chat_id=update.message.chat_id, text="Willkommen beim Attilabot"
        )
        print(id + " joined")
        dummy_person["ID"] = id
...
def inline_handler(bot, update):
        ...
        if value == "Stufe":
            ...
            row1.append(
                InlineKeyboardButton(
                    "Biber",
                    callback_data="{} {} {}".format(
                        "level", query.message.chat_id, "Biber"
                    ),
                )
            )
            row1.append(
                InlineKeyboardButton(
                    "Wölfe",
                    callback_data="{} {} {}".format(
                        "level", query.message.chat_id, "Wölfe"
                    ),
                )
            )
            row1.append(
                InlineKeyboardButton(
                    "Pfader",
                    callback_data="{} {} {}".format(
                        "level", query.message.chat_id, "Pfader"
                    ),
                )
            )
            keyboard.append(row1)
        ...
        if value == "Erinnerung":
            ...
            row1.append(
                InlineKeyboardButton(
                    "Mo",
                    callback_data="{} {} {}".format(
                        "reminder_day", query.message.chat_id, "Mo"
                    ),
                )
            )
            row1.append(
                InlineKeyboardButton(
                    "Do",
                    callback_data="{} {} {}".format(
                        "reminder_day", query.message.chat_id, "Do"
                    ),
                )
            )
            row2.append(
                InlineKeyboardButton(
                    "Fr",
                    callback_data="{} {} {}".format(
                        "reminder_day", query.message.chat_id, "Fr"
                    ),
                )
            )
            row2.append(
                InlineKeyboardButton(
                    "Sa",
                    callback_data="{} {} {}".format(
                        "reminder_day", query.message.chat_id, "Sa"
                    ),
                )
            )
    ...
    if action == "level":
        print(value)
        bot.send_message(
            chat_id=query.message.chat_id, text="Deine Stufe ist nun: {}".format(value)
        )
    ...
    if action == "reminder_day":
        ...
        row1.append(
            InlineKeyboardButton(
                "Ja",
                callback_data="{} {} {} {}".format(
                    "decision_day", query.message.chat_id, value, "Ja"
                ),
            )
        )
        row1.append(
            InlineKeyboardButton(
                "Nein",
                callback_data="{} {} {} {}".format(
                    "decision_day", query.message.chat_id, value, "Nein"
                ),
            )
        )
        keyboard.append(row1)
    ...
    if action == "decision_day":
        ...
        if arg == "Ja":
            ...
            bot.send_message(
                chat_id=query.message.chat_id, text="Ok, ich erinnere dich am " + value
            )
        ...
        if arg == "Nein":
            ...
            bot.send_message(
                chat_id=query.message.chat_id,
                text="Ok, ich erinnere dich am " + value + " nicht mehr",
            )
            change_person_value(query.message.chat_id, value, arg)
    ...
    if action == "change_activity_atribute":
        ...
        if arg != "Abbrechen" and arg != "Eingeben":
            ...
            set_activity_element(
                get_level(query.message.chat_id), value, standard_attribute[value]
            )
    ...
    if action == "change_single_atribute":
        ...
        row.append(
            InlineKeyboardButton(
                "Eingeben",
                callback_data="{} {} {} {}".format(
                    "change_activity_atribute", query.message.chat_id, value, "Eingeben"
                ),
            )
        )
        row.append(
    ...
    if action == "change_status":
        ok[arg] = value
        bot.send_message(
            chat_id=query.message.chat_id, text="Status ist jetzt: " + ok_alias[value]
        )
        ...
        if person[weekday] == "Ja":
            if ok[person["level"]] == "nope":
                updater.bot.send_message(
                    chat_id=person["ID"],
                    text="Reminder, die Webseite ist noch nicht aktuell, du "
                    + insult(),
                )
...
def get_activity(level):
    ...
    for atribute in activity_atributes:
        activity_text += (
            "<b>" + atribute + ": </b>" + get_activity_element(level, atribute) + "\n"
        )
...
def current(bot, update):
    ...
    if level is not None:
        bot.send_message(
            chat_id=update.message.chat_id, text=get_activity(level), parse_mode="HTML"
        )
    else:
        bot.send_message(
            chat_id=update.message.chat_id,
            text=r"Zerst mit \start registriere, du " + insult(),
        )
...
def current_with_id(bot, id):
    ...
    if level is not None:
        ...
        bot.send_message(
            chat_id=id, text=r"Zerst mit \start registriere, du " + insult()
        )
...
def change_activity(bot, update):
    ...
    row1.append(
        InlineKeyboardButton(
            "Alle",
            callback_data="{} {} {}".format(
                "change_all_activities", update.message.chat_id, "alle"
            ),
        )
    )
    keyboard.append(row1)
        ...
        if atribute != "Stufe":
            ...
            row.append(
                InlineKeyboardButton(
                    atribute,
                    callback_data="{} {} {}".format(
                        "change_single_atribute", update.message.chat_id, atribute
                    ),
                )
            )
            keyboard.append(row)
...
def answer_handler(bot, update):
    if update.message.chat_id not in next_message:
        bot.send_message(
            chat_id=update.message.chat_id, text="Verstandi nöd, du " + insult()
        )
        return
    ...
    if command == "Name":
        ...
        bot.send_message(
            chat_id=update.message.chat_id,
            text="Name geändert zu: {}".format(update.message.text),
        )
        if update.message.text == "Noah" or update.message.text == "Calmo":
            bot.send_message(
                chat_id=update.message.chat_id,
                text="Das ist denn ein schöner Name, Gratulation!",
            )
        else:
            bot.send_message(
                chat_id=update.message.chat_id,
                text="Was für ein hässlicher Name! Du " + insult() + "!",
            )

    elif command == "Attribut":
        set_activity_element(
            get_level(update.message.chat_id), arg, update.message.text
        )
        ...
        bot.send_message(
            chat_id=update.message.chat_id,
            text="Verstandi nöd du " + insult() + ", benutz en Command",
        )
...
def change_state(bot, update):
    ...
    row.append(
        InlineKeyboardButton(
            "Aktuell",
            callback_data="{} {} {} {}".format(
                "change_status", update.message.chat_id, "ok", level
            ),
        )
    )
    row.append(
        InlineKeyboardButton(
            "Nicht Aktuell",
            callback_data="{} {} {} {}".format(
                "change_status", update.message.chat_id, "nope", level
            ),
        )
    )
    row.append(
        InlineKeyboardButton(
            "Keine Übung",
            callback_data="{} {} {} {}".format(
                "change_status", update.message.chat_id, "later", level
            ),
        )
    )
    keyboard.append(row)
